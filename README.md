...
```
L7  PRESENTATION    mainframe bronze + modern steel (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      cobol-bridge-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — isolated mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: COBOL copybooks, CICS regions, JCL, VSAM, EBCDIC, target languages (Java/Go/Python)
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "cobolbridge"
...
```bash
git clone https://github.com/CSOAI-ORG/cobolbridge-hive
cd cobolbridge-hive
...
`https://cobolbridge.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
£199/mo Pro + £1,999/mo Enterprise + £290k/enterprise project floor.
